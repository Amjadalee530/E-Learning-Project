...
export default function AddStudentToClass() {
  ...
  useEffect(() => {
    ...
    const url2 = axios.get("/students/");
    axios
      .all([url1, url2])
      .then((responses) => {
        setClasses(responses[0].data.data);
        setStudents(responses[1].data.data);
      })
      .catch((error) =>
  ...
  const handleAddStudent = (classroomId) => {
    ...
    axios
      .put("/classrooms/" + classroomId, { studentId })
      .then(({ data }) => {
        if (data.success) {
          alert("Student successfully added to classroom");
        }
      })
      .catch((error) =>
        error.response.data.msg
                ...
                <div className="col-md-12 col-sm-12">
                  <div className="card">
                    <div className="card-header card-header-primary">
                      <h4 className="card-title ">Add Student to Class</h4>
                      {classes.length ? (
                        <div className="content">
                          <div className="container-fluid">
                            <div className="row">
